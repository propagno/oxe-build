...
const { verifyRun } = require("../verification/verification-compiler"); // Gap 2: inline verification
class Scheduler {
    constructor() {
    ...
    async runNode(nodeId, graph, ctx, status, completed, failed, blocked) {
        ...
        let lease = null;
        let lastResult = null;
        let lastError = null; // Gap 3: carry error context across retry attempts
            ...
            try {
                ...
                if (lastResult.success) {
                    ...
                    if (verifyResult && verifyResult.status === 'failed') {
                        ...
                        lastError = lastResult.output; // carry into next retry prompt
                    ...
                    // Gap 3: capture error output for next attempt's prompt
                ...
                lastError = `[error_boundary] ${message}`; // Gap 3
                if (attempt < maxAttempts) {
                    const backoffMs = Math.min(1000 * Math.pow(2, attempt - 1) + Math.random() * 500, 30000);
                    await new Promise(resolve => setTimeout(resolve, backoffMs));
                    this.emit(ctx, {
    ...
    async executeNode(node, lease, ctx, attempt, attemptId, options = {}) {
        ...
        if (!provider || !primaryAction) {
            // Gap 3: pass options (including previousError) so executor can inject retry context
    ...
    // Gap 2: run inline verification after executor succeeds
    async verifyNode(node, lease, ctx, attemptId, attempt) {
        if (!node.verify || !node.verify.command)
            return null;
        this.emit(ctx, {
        ...
        const suite = {
            checks: [{
                    ...
                    acceptance_ref: null,
                    description: `Inline verify for ${node.id}`,
                }],
        };
        let result;
        try {
            result = await verifyRun({
                ...
                evidenceStore: ctx.evidenceStore ?? null,
                pluginRegistry: ctx.pluginRegistry ?? null,
            });
        }
        catch (err) {
            // Verification infrastructure failure — do not block task progress
            this.emit(ctx, {
                type: 'VerificationCompleted',
                work_item_id: node.id,
                payload: { work_item_id: node.id, status: 'error', error: String(err) },
            });
            return null;
        }
        this.emit(ctx, {
            type: 'VerificationCompleted',
            work_item_id: node.id,
            payload: { work_item_id: node.id, status: result.status },
        });
        return result;
    }
    ...
    async requestGateForNode(node, ctx, decision) {
        if (!ctx.gateManager) {
            console.warn(`[scheduler] ctx.gateManager não configurado — gate para ${node.id} não será persistido`);
