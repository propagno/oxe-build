...
function buildNodePrompt(node, lease, runId, attempt, options = {}) {
    ...
    // Gap 3: inject previous attempt error for retry context
    if (attempt > 1 && options.previousError) {
        lines.push('', '## Contexto da tentativa anterior');
        lines.push(`Esta é a tentativa **${attempt}**. A tentativa anterior falhou com o seguinte erro:`);
        lines.push('', '```');
        lines.push(String(options.previousError).slice(0, 2000));
        lines.push('```', '');
        lines.push('Analise o erro acima e tente uma abordagem diferente.');
    }
